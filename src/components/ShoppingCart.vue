<template>
    <div>
        ...
        <div class="cart-container">
            ...
            ></CartList>
            <OrderSummary class="order-summary" :cart-items="shoppingCartItems">
        </OrderSummary>
        </div>
    </div>
</template>
...
function updateQuantity(val){
    ...
    shoppingCartItems.value.some((item)=>{
        if(item.id == id){
            item.quantity = Math.max(0,parseInt(newQuantity)) //added math.max so value cannot be less than zero
            return true
        }
    })
}
